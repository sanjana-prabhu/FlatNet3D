...
def per_image_pipeline(img, disp, filename, mean, train, device, noise_level):
    ...
    if train:
        path2 = data_dir+'train_set/meas/'
    else:
        path2 = data_dir+'val_set/meas/'
...
if Flying_things3D:
    ...
    for i in tqdm(range(train_size)):
        ...
        noise_level = np.mod(i, 5)+58
        img = np.array(Image.open(dataset_dir+'train/left/'+str(img_no)+'.png'))[::2, ::2, :]
        disp = load_pfm(dataset_dir+'train/disparity/left/'+str(img_no)+'.pfm')[::2, ::2]
        per_image_pipeline(img[:256, 100:356, :], disp[:256, 100:356], 'im'+str(img_no), np.mean(disp), True, device, noise_level)        
        
    for i in tqdm(range(3000)):
        img_no = '0'*(7-len(str(i))) + str(i)
        noise_level = np.mod(i, 5)+58
        img = np.array(Image.open(dataset_dir+'val/left/'+str(img_no)+'.png'))[::2, ::2, :]
        disp = load_pfm(dataset_dir+'val/disparity/left/'+str(img_no)+'.pfm')[::2, ::2]
        per_image_pipeline(img[:256, 100:356, :], disp[:256, 100:356], 'im'+str(img_no), np.mean(disp), False, device, noise_level)   